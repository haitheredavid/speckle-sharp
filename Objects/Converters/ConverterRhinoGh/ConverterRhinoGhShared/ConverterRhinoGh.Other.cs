...
//#endif
      ...
      if ((renderMaterial.name.ToLower().Contains("glass") || renderMaterial.name.ToLower().Contains("gem")) && renderMaterial.opacity == 0)
		...
		{
			var defaultPattern = typeof(HatchPattern.Defaults).GetProperties()?.Where(o => o.Name.Equals(patternName, StringComparison.OrdinalIgnoreCase))
		...
		public Dimension DimensionToSpeckle(Rhino.Geometry.Dimension dimension)
		{
			...
			switch (dimension)
			{
				case LinearDimension o:
					if (o.Get3dPoints(out Point3d linearStart, out Point3d linearEnd, out Point3d linearStartArrow, out Point3d linearEndArrow,
					                  out Point3d linearDimPoint, out textPoint))
					{
						var linearDimension = new DistanceDimension() { units = ModelUnits, measurement = dimension.NumericValue, isOrdinate = false };

						var normal = new Vector3d(linearEndArrow.X - linearStartArrow.X, linearEndArrow.Y - linearStartArrow.Y,
				...
				case AngularDimension o:
					if (o.Get3dPoints(out Point3d angularCenter, out Point3d angularStart, out Point3d angularEnd, out Point3d angularStartArrow,
					                  out Point3d angularEndArrow, out Point3d angularDimPoint, out textPoint))
					{
				...
				case OrdinateDimension o:
					if (o.Get3dPoints(out Point3d basePoint, out Point3d ordinateDefPoint, out Point3d leader, out Point3d kink1Point, out Point3d kink2Point))
					{
						...
						ordinateDimension.direction = Math.Round(Math.Abs(ordinateDefPoint.X - basePoint.X) - o.NumericValue) == 0
							? VectorToSpeckle(Vector3d.XAxis) : VectorToSpeckle(Vector3d.YAxis);
		...
		public Rhino.Geometry.Dimension RhinoDimensionToNative(Dimension dimension)
		{
			...
			var plane = sourceAppProps["plane"] as Plane != null ? PlaneToNative(sourceAppProps["plane"] as Plane)
			...
			switch (className)
			{
				...
				case"OrdinateDimension":
					...
					if (isXDirection)
						_dimension = OrdinateDimension.Create(dimensionStyle, plane, OrdinateDimension.MeasuredDirection.Xaxis, ordinateBase, ordinateDefining,
						                                      position, kinkOffset1, kinkOffset2);
					else
						_dimension = OrdinateDimension.Create(dimensionStyle, plane, OrdinateDimension.MeasuredDirection.Yaxis, ordinateBase, ordinateDefining,
						                                      position, kinkOffset1, kinkOffset2);
				...
				case LengthDimension o:
					switch (o.measured)
					{
						case Line l:
							...
							var radialDimension =
								RadialDimension.Create(style, AnnotationType.Radius, plane, radialLine.PointAtStart, radialLine.PointAtEnd, position);
							_dimension = radialDimension;
							break;
